...
WinstonProxyLogger.prototype.generateMetaData = function () {
  try {
    let parentMetaData = undefined;

    // Check to know if there is a parent
    if (this.properties.target instanceof WinstonProxyLogger) {
      parentMetaData = this.properties.target.generateMetaData();
    }
    ...
    if (parentMetaData || metaDataGenerated) {
      return _.merge({}, parentMetaData, metaDataGenerated);
    }
...
WinstonProxyLogger.prototype.log = function (levelName) {
  ...
  if (target) {
    let metaData = this.generateMetaData();
    ...
    if (args) {
      ...
      if (_.isObjectLike(args[logStartIndex])) {
        // We have metadata
        metaData = _.merge({}, metaData, args[logStartIndex]);
        // Remove the meta data
        _.remove(args, (value, index) => index === logStartIndex);
      }

      // The last arg
      const lastValueIndex = args.length - 1;
      if (lastValueIndex >= 0 && _.isObjectLike(args[lastValueIndex])) {
        metaData = _.merge({}, metaData, args[lastValueIndex]);
        _.remove(args, (value, index) => index === lastValueIndex);
      }
    }
